...
export default async function moviesDetails({params : {id}}) {
  const options = {
      method: 'GET',
      headers: {
    ...
    async function submitComment(e) {
      "use server"
        ...
        const sendComments = await fetch(`${process.env.APP_URL}/api/${result?.title}`,{
          method: "POST",
          cache: "no-cache",
          headers: {
            accept: 'application/json'
          },
          body: JSON.stringify({comment,movieName:result?.title})
      })
      const res = await sendComments.json()
      revalidateTag("comments")
    }
    const commentsFetch = await fetch(`${process.env.APP_URL}/api/${result?.title}`,{
      method: "GET",
       cache: "no-cache",
        headers: {
          accept: 'application/json'
    ...
    <div className="p-1.5 mt-3">
      <div className="grid grid-cols-2">
        ...
        <div className="flex flex-col items-center">
          <h1 className="font-extrabold text-center text-white text-3xl">{result?.title}</h1>
          <label className="text-white text-2xl">امتیاز : {result?.vote_average}</label>
          <label className="text-white text-2xl">زمان : {result?.runtime} دقیقه </label>
      ...
      <h1 className="text-center m-3 text-blue-200 text-3xl">{result?.release_date}</h1>
      <h1 className="text-center m-3 text-white text-3xl">وضعیت : {result?.status}</h1>
      <div>
