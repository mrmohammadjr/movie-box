...
import { MoviesFetchButton } from "../components/fetchTvMvs"
export default async function movies({searchParams}) {
  const searchArray = searchParams?.movies?.split(",")
  const genre = searchArray ? searchArray[0] : 'upcoming';
  const options = {
      method: 'GET',
      headers: {
  ...
  const res = await fetch(`https://api.themoviedb.org/3/movie/${genre}?language=en-US&page=${ searchArray ? searchArray[1] : 1}`,options)
