...
export default function TvDetails({params : {id}}) {
  ...
  const dispatch = useDispatch()
    ...
    <div className="p-1.5 mt-3">
      ...
      <h1 className="text-center m-3 text-blue-200 text-3xl">کارگردان : {data[0]?.created_by[0]?.name}</h1>
      <div className="grid j
      grid-cols-2 my-7">
        <h1 className="text-white">اولین پخش : {data[0]?.first_air_date}</h1>
        <h1 className="text-white">آخرین پخش : {data[0]?.last_air_date}</h1>
        <h1 className="text-white mt-3">وضعیت پخش : {data[0]?.status}</h1>
      </div>
      <div>
      <FavComponent param={{id:data[0]?.imdb_id,name:data[0]?.name,image:data[0]?.poster_path}}/>
      </div>
      <div className="m-1">
